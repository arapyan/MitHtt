...
#include "MitHtt/Ntupler/interface/NSVfitStandaloneAlgorithm.h"
...
NSVfitStandaloneAlgorithm::fit(){
  ...
  massUncert_ = TMath::Sqrt( 0.25*minimizer_->Errors()[kXFrac]/minimizer_->X()[kXFrac]*minimizer_->Errors()[kXFrac]/minimizer_->X()[kXFrac]+0.25*minimizer_->Errors()[kMaxFitParams+kXFrac]/minimizer_->X()[kMaxFitParams+kXFrac]*minimizer_->Errors()[kMaxFitParams+kXFrac]/minimizer_->X()[kMaxFitParams+kXFrac])*fittedDiTauSystem().M();
  ...
  if(verbosity_>1){
    ...
    std::cout << ">> mass          = " << fittedDiTauSystem().M() << std::endl;  
