...
namespace SVfit_namespace {
  ...
  TVector3 tauDirection(const TVector3&, double, double);  

  /// Compute the tau four vector given the tau direction and momentum
  TLorentzVector tauP4(const  TVector3&, double);
  ...
  double logGaussianNd(const T1& residual, const T2& cov)
  {
    ...
    if ( flag == false ) {
      std::cout << " Failed to invert covariance matrix, error flag = " << flag << " !!";
      return std::numeric_limits<float>::min();
    }
