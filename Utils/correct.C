...
TFile *fZMCFile = new TFile("/build/pharris/Recoil/zmm_phil_11.root"); // mc to be corrected
TTree *fZMCTree = (TTree*) fZMCFile->FindObjectAny("Skim");

TFile *fDataFile = new TFile("/build/pharris/Recoil/data_phil_11.root"); // data to plot with this mc
TTree *fZDataTree = (TTree*) fDataFile->FindObjectAny("Skim"); 
...
const int readRecoil(std::vector<double> &iSumEt,
		     ...
		     std::string iFName = "/build/pharris/Recoil/recoilfit.root") { // recoil fits to zmm_phil_11.root
...
// recoil fits to:            Zmm data,                             zmm MC
void correct(std::string iNameZDat = "/build/pharris/Recoil/recoilfitZDat.root",std::string iNameZMC = "/build/pharris/Recoil/recoilfitZMC.root") { 
  ...
  float lGenPt = 0; float lGenPhi = 0; float lLepPt = 0; float lLepPhi = 0; float lMet = 0; float lMPhi = 0; float lLeadingLepPt = 0;
  fZMCTree->SetBranchAddress("GenPt" ,&lGenPt);
  fZMCTree->SetBranchAddress("GenPhi",&lGenPhi);
  fZMCTree->SetBranchAddress("pt"    ,&lLepPt);
  fZMCTree->SetBranchAddress("phi"   ,&lLepPhi);
  fZMCTree->SetBranchAddress("pt_1"  ,&lLeadingLepPt);
  fZMCTree->SetBranchAddress("met"   ,&lMet);
  fZMCTree->SetBranchAddress("mphi"  ,&lMPhi);

  TH1F* lDMet = new TH1F("dMET","dMET"   ,50,0,100); lDMet->Sumw2();
  TH1F* lMMet0 = new TH1F("mMET0","mMET0",50,0,100); lMMet0->Sumw2(); lMMet0->SetLineColor(kRed);
  TH1F* lMMet1 = new TH1F("mMET1","mMET1",50,0,100); lMMet1->Sumw2(); lMMet1->SetLineColor(kBlue);
  ...
  for(int i0 = 0; i0 < fZMCTree->GetEntries(); i0++) {
    ...
    //if(pMX > 0) {pMPhi = atan(pMY/pMX);} else {pMPhi = (fabs(pMY)/pMY)*3.14159265 + atan(pMY/pMX); }
    fZMCTree->GetEntry(i0);
    if(lLeadingLepPt < 30) continue;
    double pMet  = lMet;//sqrt(pMX*pMX+pMY*pMY);
    double pMPhi = lMPhi; 
    lMMet0->Fill(pMet);
    //cout << "====> Met Before ==> " << pMet << " -- " << pMPhi ;
    metDistribution(pMet,pMPhi,lGenPt,lGenPhi,lLepPt,lLepPhi,lRandom,
...
//   lDMet->GetYaxis()->SetRangeUser(0,0.1);
