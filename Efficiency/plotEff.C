		           ...
		           const Double_t massLo, const Double_t massHi, const Bool_t doAbsEta, const Int_t charge)
{
  ...
  for(UInt_t ientry=0; ientry<intree->GetEntries(); ientry++) {
    ...
    if(data.pass) {
      ...
      passEta[ieta]->Fill(data.mass);
      passPhi[iphi]->Fill(data.mass);
      passEtaPt[ipt*etaNbins + ieta]->Fill(data.mass);
      passEtaPhi[iphi*etaNbins + ieta]->Fill(data.mass);
      passNPV[inpv]->Fill(data.mass);
      passRho[irho]->Fill(data.mass);
    } else {
      failPt[ipt]->Fill(data.mass);
      failEta[ieta]->Fill(data.mass);
      failPhi[iphi]->Fill(data.mass);
      failEtaPt[ipt*etaNbins + ieta]->Fill(data.mass);
      failEtaPhi[iphi*etaNbins + ieta]->Fill(data.mass);
      failNPV[inpv]->Fill(data.mass);
      failRho[irho]->Fill(data.mass);
    }    
  }
  infile.Close();
