...
void bbtt_et(std::string var,int nbins, double xmin, double xmax,std::string xtitle, std::string ytitle)
{
  SetStyle(); gStyle->SetLineStyleString(11,"20 10");
  TH1::SetDefaultSumw2(1);

  std::string dir = "/data/blue/Bacon/029a/etau/ntuples/mhhvar/";
  double sigscale = 10;
  double sigscale1 = 100; 
  ...
  std::string objcut = "(pt_1>24&&pt_2>30 && abs(eta_1) <2.1 && abs(eta_2)<2.3 && abs(dZ_2)<0.2 && iso_1<0.1 &&  byCombinedIsolationDeltaBetaCorrRaw3Hits_2<1.5 && passAntiEleNewWPMVA3_2>1.5 && ((pvz+130./tan(2*TMath::ATan(exp(-1.0*eta_2)))) > 0.5 || (pvz+130./tan(2*TMath::ATan(exp(-1.0*eta_2))))<-1.5))";
  std::string objcutloose = "(pt_1>24&&pt_2>30 && abs(eta_1) <2.1 && abs(eta_2)<2.3 && abs(dZ_2)<0.2 && iso_1<0.1 &&  byCombinedIsolationDeltaBetaCorrRaw3Hits_2<1.5 && passAntiEleNewWPMVA3_2>1.5 && ((pvz+130./tan(2*TMath::ATan(exp(-1.0*eta_2)))) > 0.5 || (pvz+130./tan(2*TMath::ATan(exp(-1.0*eta_2))))<-1.5))";
   std::string jetcut = objcut+"*(mjj>100 && mjj<150 && mtMVA_1<30 && jpt_1>30 && jpt_2>30 && !((jcsv_1>0.679 && jcsv_2>0.244)||(jcsv_2>0.679 && jcsv_1>0.244)) &&  !(jcsv_1>0.679 && jcsv_2>0.679) && jcsv_1>0.244 && jcsv_2>0.244)";
   std::string jetcutloose = objcutloose+"*(mjj>100 && mjj<150 && mtMVA_1<30 && jpt_1>30 && jpt_2>30 && !((jcsv_1>0.679 && jcsv_2>0.244)||(jcsv_2>0.679 && jcsv_1>0.244)) &&  !(jcsv_1>0.679 && jcsv_2>0.679) && jcsv_1>0.244 && jcsv_2>0.244)";
  ...
  std::string tthcut = jetcut+"*(weight*decaymodeweight*0.0789/1.972e09)*(q_1*q_2<0)*"+lumi.str();
  std::string vbfcut = jetcut+"*(weight*decaymodeweight*0.0997/1.972e09)*(q_1*q_2<0)*"+lumi.str();
  std::string gfcut = jetcut+"*(weight*decaymodeweight*1.2179/1.972e09)*(q_1*q_2<0)*"+lumi.str();
  std::string zttcut = jetcut+"*weight*(decaymodeweight/1.972e09)*emblid*(genmatch==3)*(q_1*q_2<0)";
  std::string ttbarcut = jetcut+"*1.11*0.96*(q_1*q_2<0)*(weight*decaymodeweight/1.972e09)*"+lumi.str();
  std::string wjetcut = jetcut+"*(q_1*q_2<0)*(weight*decaymodeweight/1.972e09)*"+lumi.str();
  std::string ewkcut = jetcut+"*(q_1*q_2<0)*(weight*decaymodeweight/1.972e09)*"+lumi.str();
  std::string zttmccut = jetcut+"*weight*(decaymodeweight/1.972e09)*(genmatch==3)*(q_1*q_2<0)*"+lumi.str();
  std::string zllcut = jetcut+"*weight*zeefrate*(decaymodeweight/1.972e09)*(abs(genlid_1)<15 && abs(genlid_2)<15 && genmatch==1)*(q_1*q_2<0)*"+lumi.str();
  std::string zljcut = jetcut+"*weight*(decaymodeweight/1.972e09)*(genmatch!=3 && genmatch!=1)*(q_1*q_2<0)*"+lumi.str();
  std::string fakescut = jetcut+"*1.06*(q_1*q_2>0)";
  //Ztt normalization
  std::string zttcutloose = objcut+"*weight*(decaymodeweight/1.972e09)*emblid*(genmatch==3)*(q_1*q_2<0)";
  std::string zttmccutloose = objcut+"*weight*(decaymodeweight/1.972e09)*(genmatch==3)*(q_1*q_2<0)*"+lumi.str();
  //QCD SS control region
  std::string fakescutloose = jetcutloose+"*1.06*(q_1*q_2>0)";
  std::string zttmccutss = jetcutloose+"*weight*(decaymodeweight/1.972e09)*(genmatch==3)*(q_1*q_2>0)*"+lumi.str();
   std::string zllcutss = jetcutloose+"*weight*(decaymodeweight/1.972e09)*(abs(genlid_1)<15 && abs(genlid_2)<15 && genmatch==1)*(q_1*q_2>0)*"+lumi.str();
  std::string zljcutss = jetcutloose+"*weight*(decaymodeweight/1.972e09)*(genmatch!=3 && genmatch!=1)*(q_1*q_2>0)*"+lumi.str();
  std::string ttbarcutss = jetcutloose+"*1.11*0.96*(q_1*q_2>0)*(weight*decaymodeweight/1.972e09)*"+lumi.str();
  std::string ewkcutss = jetcutloose+"*(q_1*q_2>0)*(weight*decaymodeweight/1.972e09)*"+lumi.str();
  std::string wjetcutss = jetcutloose+"*(q_1*q_2>0)*(weight*decaymodeweight/1.972e09)*"+lumi.str();
  //--------------------------------------------------------------------------
  ...
  TTree *zeetree = load(dir+"zee_select.root");
  ...
  TTree *smhhtree = load(dir+"s12-hh125-bbtt-et-8tev_select.root");
  ...
  vardraw = var+">>"+"Zll";
  zeetree->Draw(vardraw.c_str(),zllcut.c_str());
  ...
  //qcd SS control region
  TH1F *fakesloose = new TH1F("FakesLoose","",nbins,xmin,xmax);
  vardraw = var+">>"+"FakesLoose";
  datatree->Draw(vardraw.c_str(),fakescutloose.c_str());
  InitHist(fakesloose, xtitle.c_str(), ytitle.c_str(),  TColor::GetColor(250,202,255), 1001);
  ...
  fakesloose->Add(ssbkg,-1);
  for(int ibin = 0; ibin < fakesloose->GetNbinsX(); ibin++) if(fakesloose->GetBinContent(ibin) < 0) fakesloose->SetBinContent(ibin,0);
  fakesloose->Scale(fakesloose->Integral()/fakesloose->Integral());
  fakes=fakesloose;
  ...
  outfile << "Zll    "  << zllss->IntegralAndError(0,zllss->GetNbinsX(),error) << "+/-" << error << endl;
   ...
   /*
     Ratio Data over MC
   */
   TCanvas *canv1 = MakeCanvas("canv0", "histograms", 600, 200);
   canv1->SetGridx();
   canv1->SetGridy();
   canv1->cd();
