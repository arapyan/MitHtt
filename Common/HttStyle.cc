...
{
  TPaveText* cmsprel  = new TPaveText(lowX, lowY+0.06, lowX+0.5, lowY+0.16, "NDC");
  ...
  TPaveText* lumi     = new TPaveText(lowX, lowY+0.06, lowX+0.5, lowY+0.08, "NDC");
  ...
  lumi->SetTextColor(    1 );
  lumi->SetTextFont (   62 );
  lumi->AddText("4.0 fb^{-1}  #sqrt{s}=7 TeV");
  lumi->Draw();
