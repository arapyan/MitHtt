...
void rescale(TH1F* hin, unsigned int idx)
{
  ...
  double CMS_scale_j             = 0.9620; // -0.76 * 1.05
  double CMS_htt_zttNorm         = 1.3180; // +1.06 * 1.30
  double CMS_htt_ttbarNorm       = 0.9350; // -0.65 * 1.10
  double CMS_htt_DiBosonNorm     = 0.7840; // -0.72 * 1.30
  double CMS_hww_fakes_em        = 0.8950; // -0.21 * 1.50
...
emuAfterFit_vbf(bool scaled = true)
{
  ...
  TH1F* ggH   =  refill((TH1F*)exampleFile->Get("emu_vbf/Higgs_gf_sm_120"  )); InitSignal(ggH); ggH->Scale(10*16.63*0.071*0.0133/ggH->Integral());
  TH1F* qqH   =  refill((TH1F*)exampleFile->Get("emu_vbf/Higgs_vbf_sm_120" )); InitSignal(qqH); qqH->Scale(10*1.269*0.071*0.1306/qqH->Integral());
