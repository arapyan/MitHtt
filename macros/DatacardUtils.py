...
def parse_dcard(datacard,fitres,bin_name="ANYBIN"):
    ...
    # The second to last section should include a row that starts with "process" and has columns for each sample name, separated by whitespace
    # The last section consists of rows in the following format: parameter_name fit_type uncert_1 uncert_2 uncert_3 ...., where uncert_1 is the 
    # value of a +1 sigma variation in the parameter
    # Only parameters of type "lnN" are considered
    # The number of sigma variations of a given parameter in the ml fit is given in the fit result file, fitres; this value is obtained by calling
    # DatacardUtils.get_svar (see above)
    # The size of the sigma variation is multiplied by the number of sigma variations and added to 1 in order to get the fit weight
    # The resulting dictionary is the list of fit weights for the category specified by bin_name (or else the only category represented in the sample if bin_name="ANYBIN" is specified
