...
void selectEmu(const TString conf,         // input config file
  ...
  for(UInt_t isam=0; isam<samplev.size(); isam++) {
	...
	Double_t idscale = 1;
	if(doIdScale) idscale = muIDscaleEmu(mu->pt,mu->eta,is2012)*eleIDscaleEmu(ele->pt,ele->eta,is2012);

	// trigger scale factor for MC
	Double_t trigscale = 1;
	if(doTrigScale && !isemb) trigscale=muTrigScaleEmu(mu->pt,mu->eta,is2012)*eleTrigScaleEmu(ele->pt,ele->eta,is2012);
