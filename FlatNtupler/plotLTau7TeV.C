...
void drawSpec(TTree **iTree,TH1F **iHMM,TH1F **iHMMSS,TH1F **iHMMMT,TH1F **iHMMSST,int iN,std::string iVar,int iCat,TFile * iFile = 0,std::string iDirName="",int iUseScale=0,int *ptrew=0,TH1F** iHHigh=0,TH1F** iHLow=0,TH1F** iHHVBF=0,TH1F** iHSST=0,TH1F** iHTheory=0,TH1F **iLTheory=0) { 
  ...
  for(int i0 = 0; i0 < iN; i0++) {
	...
	//notes:  iHMMSS->W shape, iHMMMT/(iHSST)->W norm, iHMMMST->QCD norm, qcd shape-->iHMMMST 
      ...
      {
	iHMM   [i0]   = draw(iVar,iCat,iTree[i0],i0,"*(jpt_2>30 && byCombinedIsolationDeltaBetaCorrRaw3Hits_2<1.5 && jdeta>3.5 && mjj>500 && njetingap==0)*(q_1*q_2 < 0)*(mtMVA_1<30)*(iso_1<0.1)"," Main");
	...
	if(i0!=fWId)
	  iHMMMT [i0]   = draw(iVar,iCat,iTree[i0],i0,"*(jpt_2>30 && jdeta>3.5 && mjj>500 && njetingap==0 && byCombinedIsolationDeltaBetaCorrRaw3Hits_2<1.5)*(mtMVA_1>60 && mtMVA_1<120)*(iso_1<0.1)*(q_1*q_2<0)","mT sideband");
	else
	  iHMMMT [i0]   = draw(iVar,iCat,iTree[i0],i0,"*(mtMVA_1>60 && mtMVA_1<120 && byCombinedIsolationDeltaBetaCorrRaw3Hits_2<1.5)*(iso_1<0.1)","mT sideband");
	...
	iHSST [i0]   = draw(iVar,iCat,iTree[i0],i0,"*(mtMVA_1<30 && byCombinedIsolationDeltaBetaCorrRaw3Hits_2<1.5)*(iso_1<0.1)","Same Sign Iso");
	iHMMSST[i0]   = draw(iVar,iCat,iTree[i0],i0,"*(q_1*q_2 > 0)*(mtMVA_1<30)*(iso_1>0.2 && iso_1<0.5 && byCombinedIsolationDeltaBetaCorrRaw3Hits_2<1.5)*(jpt_2>30 && jdeta>3.5 && mjj>500 && njetingap==0)"," Same Sign mT Sideband");
	//notes:  iHSST->W shape, iHMMMT/(iHSST)->W norm, iHMMMST->QCD norm, qcd shape-->iHMMSS
      }
    else if (iCat==7)
      {
	iHMM   [i0]   = draw(iVar,iCat,iTree[i0],i0,"*(jpt_1>30 && nbtag==0 && pt_2>45 && pthmva>100 && !(mjj > 500 && jdeta > 3.5 && njetingap==0) && byCombinedIsolationDeltaBetaCorrRaw3Hits_2<1.5)*(q_1*q_2 < 0)*(mtMVA_1<30)*(iso_1<0.1)"," Main");
	if(i0==fWId)
	  iHSST  [i0]   = draw(iVar,iCat,iTree[i0],i0,"*(jpt_1>30 && nbtag==0 && pt_2>45 && pthmva>100 && !(mjj > 500 && jdeta > 3.5 && njetingap==0) && byCombinedIsolationDeltaBetaCorrRaw3Hits_2<10)*(q_1*q_2 < 0)*(mtMVA_1<30)*(iso_1<0.1)","Same Sign Iso");
	else
	...
	iHMMMT [i0]   = draw(iVar,iCat,iTree[i0],i0,"*(jpt_1>30 && nbtag==0 && pt_2>45 && pthmva>100 && !(mjj > 500 && jdeta > 3.5 && njetingap==0) && byCombinedIsolationDeltaBetaCorrRaw3Hits_2<1.5)*(q_1*q_2 < 0)*(mtMVA_1>70)*(iso_1<0.1)","mT sideband");	
	iHMMSST[i0]   = draw(iVar,iCat,iTree[i0],i0,"*(jpt_1>30 && nbtag==0 && pt_2>45 && pthmva>100 && !(mjj > 500 && jdeta > 3.5 && njetingap==0) && byCombinedIsolationDeltaBetaCorrRaw3Hits_2<10.0)*(mtMVA_1<30)*(q_1*q_2 > 0)*(iso_1<0.1)","Wjets shape");
	iHMMSS [i0]   = draw(iVar,iCat,iTree[i0],i0,"*(jpt_1>30 && nbtag==0 && pt_2>45 && pthmva>100 && !(mjj > 500 && jdeta > 3.5 && njetingap==0) && byCombinedIsolationDeltaBetaCorrRaw3Hits_2<10.0)*(q_1*q_2 > 0)*(mtMVA_1<30)*(iso_1>0.2 && iso_1<0.5)","Same Sign Loose Iso");
	//notes:  iHMMMST->W shape, iHMMMT/(iHMM)->W norm, iHMMMST->QCD norm, qcd shape-->iHMMSS
      }
    else
      {
	iHMM   [i0]   = draw(iVar,iCat,iTree[i0],i0,"*(q_1*q_2 < 0 && byCombinedIsolationDeltaBetaCorrRaw3Hits_2<1.5)*(mtMVA_1<30)*(iso_1<0.1)"," Main");
	iHSST  [i0]   = draw(iVar,iCat,iTree[i0],i0,"*(q_1*q_2 > 0 && byCombinedIsolationDeltaBetaCorrRaw3Hits_2<1.5)*(mtMVA_1<30)*(iso_1<0.1)","Same Sign Iso");
	iHMMMT [i0]   = draw(iVar,iCat,iTree[i0],i0,"*(q_1*q_2 < 0)*(mtMVA_1>70 && byCombinedIsolationDeltaBetaCorrRaw3Hits_2<1.5)*(iso_1<0.1)","mT sideband");	
	iHMMSST[i0]   = draw(iVar,iCat,iTree[i0],i0,"*(q_1*q_2 > 0)*(mtMVA_1>70 && byCombinedIsolationDeltaBetaCorrRaw3Hits_2<1.5)*(iso_1<0.1)"," Same Sign mT Sideband");
	if(i0==iN-1 && (iCat==5 || iCat==6))
	  iHMMSS [i0]   = draw(iVar,iCat,iTree[i0],i0,"*(q_1*q_2 > 0)*(mtMVA_1<30)*(iso_1>0.2 && iso_1<0.5 && byCombinedIsolationDeltaBetaCorrRaw3Hits_2<10)","Same Sign Loose Iso");
	else
	  iHMMSS [i0]   = draw(iVar,iCat,iTree[i0],i0,"*(q_1*q_2 > 0)*(mtMVA_1<30)*(iso_1>0.2 && iso_1<0.5 && byCombinedIsolationDeltaBetaCorrRaw3Hits_2<1.5)","Same Sign Loose Iso");
	...
	if(i0==3)
	  {
	    iHHigh[i0]   = draw(iVar+"*1.02",iCat,iTree[i0],i0,"*(q_1*q_2 < 0)*(mtMVA_1<30)*(iso_1<0.1 && byCombinedIsolationDeltaBetaCorrRaw3Hits_2<1.5)"," Main");
	    iHLow [i0]   = draw(iVar+"*0.98",iCat,iTree[i0],i0,"*(q_1*q_2 < 0)*(mtMVA_1<30)*(iso_1<0.1 && byCombinedIsolationDeltaBetaCorrRaw3Hits_2<1.5)"," Main");
	  }
	else
	  {
	...
	if(i0==3)
	  ...
	  {
	    iHHigh[i0]   = draw(iVar+"high",iCat,iTree[i0],i0,"*(jpt_2>30 && jdeta>3.5 && mjj>500 && njetingap==0)*(q_1*q_2 < 0)*(mtMVA_1<30)*(iso_1<0.1 && byCombinedIsolationDeltaBetaCorrRaw3Hits_2<1.5)"," Main",1);
	    iHLow [i0]   = draw(iVar+"low",iCat,iTree[i0],i0,"*(jpt_2>30 && jdeta>3.5 && mjj>500 && njetingap==0)*(q_1*q_2 < 0)*(mtMVA_1<30)*(iso_1<0.1 && byCombinedIsolationDeltaBetaCorrRaw3Hits_2<1.5)"," Main",2);
	  }
	if(ptrew[i0])
	  {
	    iHTheory[i0]   = draw(iVar,iCat,iTree[i0],i0,"*(jpt_2>30 && jdeta>3.5 && mjj>500 && njetingap==0)*(q_1*q_2 < 0)*(mtMVA_1<30)*(iso_1<0.1 && byCombinedIsolationDeltaBetaCorrRaw3Hits_2<1.5)"," Main",0,1);
	    iLTheory[i0]   = draw(iVar,iCat,iTree[i0],i0,"*(jpt_2>30 && jdeta>3.5 && mjj>500 && njetingap==0)*(q_1*q_2 < 0)*(mtMVA_1<30)*(iso_1<0.1 && byCombinedIsolationDeltaBetaCorrRaw3Hits_2<1.5)"," Main",0,2);
	  }
      }
    else if (iCat==7)
      {
	if(i0==3)
	  {
	    iHHigh[i0]   = draw(iVar+"*1.02",iCat,iTree[i0],i0,"*(jpt_1>30 && nbtag==0 && pt_2>45 && pthmva>100 && !(mjj > 500 && jdeta > 3.5 && njetingap==0) && byCombinedIsolationDeltaBetaCorrRaw3Hits_2<1.5)*(q_1*q_2 < 0)*(mtMVA_1<30)*(iso_1<0.1)"," Main");
	    iHLow[i0]   = draw(iVar+"*0.98",iCat,iTree[i0],i0,"*(jpt_1>30 && nbtag==0 && pt_2>45 && pthmva>100 && !(mjj > 500 && jdeta > 3.5 && njetingap==0) && byCombinedIsolationDeltaBetaCorrRaw3Hits_2<1.5)*(q_1*q_2 < 0)*(mtMVA_1<30)*(iso_1<0.1)"," Main");
	  }
	else
	  {
	    iHHigh[i0]   = draw(iVar+"high",iCat,iTree[i0],i0,"*(jpt_1>30 && nbtag==0 && pt_2>45 && pthmva>100 && !(mjj > 500 && jdeta > 3.5 && njetingap==0) && byCombinedIsolationDeltaBetaCorrRaw3Hits_2<1.5)*(q_1*q_2 < 0)*(mtMVA_1<30)*(iso_1<0.1)"," Main",1);
	    iHLow[i0]   = draw(iVar+"low",iCat,iTree[i0],i0,"*(jpt_1>30 && nbtag==0 && pt_2>45 && pthmva>100 && !(mjj > 500 && jdeta > 3.5 && njetingap==0) && byCombinedIsolationDeltaBetaCorrRaw3Hits_2<1.5)*(q_1*q_2 < 0)*(mtMVA_1<30)*(iso_1<0.1)"," Main",2);
	  }
	if(ptrew[i0])
	  {
	    iHTheory[i0]   = draw(iVar,iCat,iTree[i0],i0,"*(jpt_1>30 && nbtag==0 && pt_2>45 && pthmva>100 && !(mjj > 500 && jdeta > 3.5 && njetingap==0) && byCombinedIsolationDeltaBetaCorrRaw3Hits_2<1.5)*(q_1*q_2 < 0)*(mtMVA_1<30)*(iso_1<0.1)"," Main",0,1);
	    iLTheory[i0]   = draw(iVar,iCat,iTree[i0],i0,"*(jpt_1>30 && nbtag==0 && pt_2>45 && pthmva>100 && !(mjj > 500 && jdeta > 3.5 && njetingap==0) && byCombinedIsolationDeltaBetaCorrRaw3Hits_2<1.5)*(q_1*q_2 < 0)*(mtMVA_1<30)*(iso_1<0.1)"," Main",0,2);
	  }
      }
    else
      {
  ...
  if(iCat==2 || iCat==3 || iCat==10)
    ...
    {
      double qinc = 1045.93;  //to be determined from inclusive
      double qssinc =17102;  //tau iso relaxed
...
//       iHMM[0]->SetBinError(i0,1.0);
  ...
  TH1F* llll = (TH1F *) iHMMMT[fQCDId]->Clone("data");
  TH1F* lllw = (TH1F *) iHMMMT[iN-1]->Clone("ww");
  iHMMMT[iN-1]=llll;
  //iHMMMT[fWId]=lllw;
  iHMMMT[fQCDId]->Scale(0);
  //Draw the plot
...
void plotLTau7TeV(std::string iVar="m_vis",int iCat = 0,int makecard=0,TFile *outfile=0,std::string iCut="(pt_1 > 45 && pt_2 > 45 && iso_1 > 0.5 && iso_2 > 0.5 && jpt_1 > 30.0 && abs(jeta_1) < 3.0 ",float iLumi=12000) { 
  SetStyle();  
  ...
  std::string lName  = "/data/blue/Bacon/029a/mutau7TeV/ntuples/";
  ...
  //lName = "/data/blue/Bacon/029a/mutau/ntuples/mutau_corr/";
  lTree[0]  = load(lName+"emb_select.root");        fString[0] = "Z#rightarrow#tau#tau ";   fCard[0]="ZTT";       fColor[0] = kOrange-4;//kOrange-3; 
  //lName  = "/data/blue/Bacon/029a/mutau/ntuples/";
  lTree[1]  = load(lName+"ttbar-7TeV_select.root");     fString[1] = "t#bar{t}";      fCard[1]="TT";                 fColor[1] = kBlue-8;//kRed+4;
  ...
  lTree[4]  = load(lName+"ewk-7TeV_select.root");            fString[4] = "electroweak";    fCard[4]="VV";  fColor[4] = kRed+2;
  ...
  lTree[7]  = load(lName+"vbf_sm_125_select.root");         fString[7] = "Higgs ";     fCard[7]="qqH125";   fColor[7] = 0;
  lTree[8]  = load(lName+"vtth_sm_125_select.root");         fString[8] = "Higgs ";     fCard[8]="VH125";    fColor[8] = 0;
  //lName = "/data/blue/Bacon/029a/mutau/ntuples/";
  ...
  if(makecard)
    {
      ...
      lTree[10]  = load(lName+"vbf_sm_110_select.root");         fString[10] = "Higgs ";     fCard[10]="qqH110";  
      lTree[11]  = load(lName+"vtth_sm_110_select.root");         fString[11] = "Higgs ";     fCard[11]="VH110"; 
      lTree[12]  = load(lName+"higgsrw/Scalehtt_gf_sm_115_select.root");         fString[12] = "Higgs ";     fCard[12]="ggH115";   ptrew[12]=1;
      lTree[13]  = load(lName+"vbf_sm_115_select.root");         fString[13] = "Higgs ";     fCard[13]="qqH115";  
      lTree[14]  = load(lName+"vtth_sm_115_select.root");         fString[14] = "Higgs ";     fCard[14]="VH115";
      lTree[15]  = load(lName+"higgsrw/Scalehtt_gf_sm_120_select.root");         fString[15] = "Higgs ";     fCard[15]="ggH120";   ptrew[15]=1;
      lTree[16]  = load(lName+"vbf_sm_120_select.root");         fString[16] = "Higgs ";     fCard[16]="qqH120";  
      lTree[17]  = load(lName+"vtth_sm_120_select.root");         fString[17] = "Higgs ";     fCard[17]="VH120"; 
      lTree[18]  = load(lName+"higgsrw/Scalehtt_gf_sm_130_select.root");         fString[18] = "Higgs ";     fCard[18]="ggH130";   ptrew[18]=1;
      lTree[19]  = load(lName+"vbf_sm_130_select.root");         fString[19] = "Higgs ";     fCard[19]="qqH130";  
      lTree[20]  = load(lName+"vtth_sm_130_select.root");         fString[20] = "Higgs ";     fCard[20]="VH130";
      lTree[21]  = load(lName+"higgsrw/Scalehtt_gf_sm_135_select.root");         fString[21] = "Higgs ";     fCard[21]="ggH135";  ptrew[21]=1;
      lTree[22]  = load(lName+"vbf_sm_135_select.root");         fString[22] = "Higgs ";     fCard[22]="qqH135";  
      lTree[23]  = load(lName+"vtth_sm_135_select.root");         fString[23] = "Higgs ";     fCard[23]="VH135";
      lTree[24]  = load(lName+"higgsrw/Scalehtt_gf_sm_140_select.root");         fString[24] = "Higgs ";     fCard[24]="ggH140";   ptrew[24]=1;
      lTree[25]  = load(lName+"vbf_sm_140_select.root");         fString[25] = "Higgs ";     fCard[25]="qqH140";  
      lTree[26]  = load(lName+"vtth_sm_140_select.root");         fString[26] = "Higgs ";     fCard[26]="VH140"; 
      lTree[27]  = load(lName+"higgsrw/Scalehtt_gf_sm_145_select.root");         fString[27] = "Higgs ";     fCard[27]="ggH145";  ptrew[27]=1;
      lTree[28]  = load(lName+"vbf_sm_145_select.root");         fString[28] = "Higgs ";     fCard[28]="qqH145";  
      lTree[29]  = load(lName+"vtth_sm_145_select.root");         fString[29] = "Higgs ";     fCard[29]="VH145"; 
      ...
      lTree[32]  = load(lName+"vbf_sm_105_select.root");         fString[32] = "Higgs ";     fCard[32]="qqH105";  
      lTree[33]  = load(lName+"vtth_sm_105_select.root");         fString[33] = "Higgs ";     fCard[33]="VH105";
      lTree[34]  = load(lName+"higgsrw/Scalehtt_gf_sm_100_select.root");         fString[34] = "Higgs ";     fCard[34]="ggH100";  ptrew[34]=1;
      lTree[35]  = load(lName+"vbf_sm_100_select.root");         fString[35] = "Higgs ";     fCard[35]="qqH100";  
      lTree[36]  = load(lName+"vtth_sm_100_select.root");         fString[36] = "Higgs ";     fCard[36]="VH100";
      lTree[37]  = load(lName+"higgsrw/Scalehtt_gf_sm_95_select.root");         fString[37] = "Higgs ";     fCard[37]="ggH95";  ptrew[37]=1;
      lTree[38]  = load(lName+"vbf_sm_95_select.root");         fString[38] = "Higgs ";     fCard[38]="qqH95";  
      lTree[39]  = load(lName+"vtth_sm_95_select.root");         fString[39] = "Higgs ";     fCard[39]="VH95";
      lTree[40]  = load(lName+"higgsrw/Scalehtt_gf_sm_90_select.root");         fString[40] = "Higgs ";     fCard[40]="ggH90";  ptrew[40]=1;
      lTree[41]  = load(lName+"vbf_sm_90_select.root");         fString[41] = "Higgs ";     fCard[41]="qqH90";  
      lTree[42]  = load(lName+"vtth_sm_90_select.root");         fString[42] = "Higgs ";     fCard[42]="VH90";
      lTree[43]  = load(lName+"higgsrw/Scalehtt_gf_sm_150_select.root");         fString[43] = "Higgs ";     fCard[43]="ggH150";  ptrew[43]=1;
      lTree[44]  = load(lName+"vbf_sm_150_select.root");         fString[44] = "Higgs ";     fCard[44]="qqH150";  
      lTree[45]  = load(lName+"vtth_sm_150_select.root");         fString[45] = "Higgs ";     fCard[45]="VH150";
      lTree[46]  = load(lName+"higgsrw/Scalehtt_gf_sm_155_select.root");         fString[46] = "Higgs ";     fCard[46]="ggH155";  ptrew[46]=1;
      lTree[47]  = load(lName+"vbf_sm_155_select.root");         fString[47] = "Higgs ";     fCard[47]="qqH155";  
      lTree[48]  = load(lName+"vtth_sm_155_select.root");         fString[48] = "Higgs ";     fCard[48]="VH155";
      lTree[49]  = load(lName+"higgsrw/Scalehtt_gf_sm_160_select.root");         fString[49] = "Higgs ";     fCard[49]="ggH160";  ptrew[49]=1;
      lTree[50]  = load(lName+"vbf_sm_160_select.root");         fString[50] = "Higgs ";     fCard[50]="qqH160";  
      lTree[51]  = load(lName+"vtth_sm_160_select.root");         fString[51] = "Higgs ";     fCard[51]="VH160";
    }
  ...
  if(iCat==0) 
    ...
    iCut=iCut+"  && nbtag==0 && jpt_2>30 &&  mjj>200 && jdeta>2.0 && njetingap==0)";
    dir="muTau_vbf";
  }
  else if(iCat==2)
    {
    ...
    {
      iCut=iCut+"  && jpt_1<30 && nbtag==0 && pt_2>30 && pt_2<45 && byCombinedIsolationDeltaBetaCorrRaw3Hits_2<1.5)"; dir="muTau_0jet_medium";
    }
  else if(iCat==4)
    {
      iCut=iCut+"  && jpt_1<30 && nbtag==0 && pt_2>45 && byCombinedIsolationDeltaBetaCorrRaw3Hits_2<1.5)"; dir="muTau_0jet_high";
    }
  else if(iCat==5)
    {
      iCut=iCut+"  && jpt_1>30 && nbtag==0 && pt_2<45 && pt_2>30 && !(jpt_2>30 && mjj > 500 && jdeta > 3.5 && njetingap==0) && byCombinedIsolationDeltaBetaCorrRaw3Hits_2<10)"; dir="muTau_1jet_medium";
    }
  else if(iCat==6)
    {
      iCut=iCut+"  && jpt_1>30 && nbtag==0 && pt_2>45 && pthmva<100 && !(jpt_2>30 && mjj > 500 && jdeta > 3.5 && njetingap==0) && byCombinedIsolationDeltaBetaCorrRaw3Hits_2<10)"; dir="muTau_1jet_high_lowhiggs";
    }
  else if(iCat==7)
    {
      iCut=iCut+"  && 1)"; dir="muTau_1jet_high_mediumhiggs";
    }
  else {
  ...
  //fWeights[0]="(pt_1 > 20 && pt_2 > 20 &&  abs(eta_1) <2.1 && abs(eta_2)<2.3 &&  abs(dZ_2)<0.2)*weight*decaymodeweight*embkinweight";
  //fWeights[3]="(pt_1 > 20 && pt_2 > 20 && abs(eta_1) <2.1  && abs(eta_2)<2.3 &&  abs(dZ_2)<0.2)*weight*decaymodeweight*(abs(genlid_1)>=15 && abs(genlid_2)>=15)";
  fWeights[0]="(pt_1 > 17 && pt_2 > 30 &&  abs(eta_1) <2.1 && abs(eta_2)<2.1 &&  abs(dZ_2)<0.2)*weight*decaymodeweight*emblid*(genmatch==3)";
  fWeights[3]="(pt_1 > 17 && pt_2 > 30 && abs(eta_1) <2.1  && abs(eta_2)<2.1 &&  abs(dZ_2)<0.2)*weight*decaymodeweight*(genmatch==3)";
  ...
  //Z Scale Factors
  fWeights[0]  += "*0.95*emblid*(genmatch==3)";
  fWeights[1]  += "*0.95"; //0.605
  if(iCat==4)
    fWeights[2]  += "*1.0";
  else
    fWeights[2]  += "*1.0";
  fWeights[5]  += "*1.0";
  if(makecard)
    fWeights[3]  += "*0.95*(abs(genlid_1)<15 && abs(genlid_2)<15 && genmatch==1)";
  else
    fWeights[3]  += "*0.95*(abs(genlid_1)<15 && abs(genlid_2)<15)";
  ...
  if(makecard) 
    {
      fWeights[30]  += "*0.95*(genmatch!=3 && genmatch!=1)";
      cout << "Hey Hey" << endl;
    }
