...
void drawSpec(TTree **iTree,TH1F **iHMM,TH1F **iHMMSS,TH1F **iHMMMT,TH1F **iHMMSST,int iN,std::string iVar,int iCat,TFile * iFile = 0,std::string iDirName="",int iUseScale=0,int *ptrew=0,TH1F** iHHigh=0,TH1F** iHLow=0,TH1F** iHHVBF=0,TH1F** iHSST=0,TH1F** iHTheory=0,TH1F **iLTheory=0) { 
  ...
  for(int i0 = 0; i0 < iN; i0++) {
	...
	//notes:  iHMMSS->W shape, iHMMMT/(iHSST)->W norm, iHMMMST->QCD norm, qcd shape-->iHMMMST 
      ...
      {
	iHMM   [i0]   = draw(iVar,iCat,iTree[i0],i0,"*(jpt_2>30 && byCombinedIsolationDeltaBetaCorrRaw3Hits_2<1.5 && jdeta>3.5 && mjj>500 && njetingap==0 && !(jpt_2>30 && jdeta>4.0 && njetingap==0 &&  mjj>700 && pthmva>100))*(q_1*q_2 < 0)*(mtMVA_1<30)*(iso_1<0.1)"," Main");
	...
	if(i0!=fWId)
	  iHMMMT [i0]   = draw(iVar,iCat,iTree[i0],i0,"*(jpt_2>30 && jdeta>3.5 && mjj>500 && njetingap==0 && byCombinedIsolationDeltaBetaCorrRaw3Hits_2<1.5 && !(jpt_2>30 && njetingap==0 && jdeta>4.0 && mjj>700 && pthmva>100))*(mtMVA_1>60 && mtMVA_1<120)*(iso_1<0.1)*(q_1*q_2<0)","mT sideband");
	else
	...
	iHMMSST[i0]   = draw(iVar,iCat,iTree[i0],i0,"*(q_1*q_2 > 0)*(mtMVA_1<30)*(iso_1>0.2 && iso_1<0.5 && byCombinedIsolationDeltaBetaCorrRaw3Hits_2<1.5)*(jpt_2>30 && jdeta>3.5 && mjj>500 && njetingap==0 && !(jpt_2>30 && jdeta>4.0 && njetingap==0 && mjj>700 && pthmva>100))"," Same Sign mT Sideband");
  ...
  if(iCat==2 || iCat==3 || iCat==10)
    ...
    {
      double qinc = 4708.52;  //to be determined from inclusive
      double qssinc =58593;  //tau iso relaxed
...
void plotLTau(std::string iVar="m_vis",int iCat = 0,int makecard=0,TFile *outfile=0,std::string iCut="(pt_1 > 45 && pt_2 > 45 && iso_1 > 0.5 && iso_2 > 0.5 && jpt_1 > 30.0 && abs(jeta_1) < 3.0 ",float iLumi=12000) { 
  SetStyle();  
  ...
  fWeights[0]="(pt_1 > 20 && pt_2 > 30 &&  abs(eta_1) <2.1 && abs(eta_2)<2.1 &&  abs(dZ_2)<0.2)*weight*decaymodeweight*emblid*(genmatch==3)";
  fWeights[3]="(pt_1 > 20 && pt_2 > 30 && abs(eta_1) <2.1  && abs(eta_2)<2.1 &&  abs(dZ_2)<0.2)*weight*decaymodeweight*(genmatch==3)";
  ...
  //Z Scale Factors
  fWeights[0]  += "*1.00*emblid*(genmatch==3)";
  ...
  fWeights[5]  += "*1.0";
  if(makecard)
    fWeights[3]  += "*1.00*(abs(genlid_1)<15 && abs(genlid_2)<15 && genmatch==1)";
  else
    fWeights[3]  += "*1.00*(abs(genlid_1)<15 && abs(genlid_2)<15)";
