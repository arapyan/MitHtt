...
TH1F* draw(TTree *iTree,int iId,int iType,TH1F *iH,std::string iVar,std::string iCut,std::string iSId) { 
  ...
  std::string lMTCut   = "(pt_1 > 10 && pt_2 > 10 && abs(eta_1) < 2.1 && abs(eta_2) < 2.3  && mt_1 > 50                   )*"+fWeights[iId]+iCut;
  TH1F *lMet = iH;
  if(iH == 0) { 
    lMet = new TH1F(lName.str().c_str(),lName.str().c_str(),getNBins(lProjMet),getXMin(lProjMet),getXMax(lProjMet)); 
    if(iId != 6 && iId != 7 && iId != 5) lMet->SetFillColor(fColor[iId]);
    if(iId == 6 || iId == 7 || iId == 5) lMet->SetLineColor(fColor[iId]);
    lMet->SetLineWidth(1);
  ...
  if(iType == -1) cout << " ===> " << iId << " --> " << lCut << "--> " << lMet->Integral() << endl;
  return lMet;
}
TLegend * legend(int iN,TH1** iH) {
  ...
  for(int i0 = 0; i0 < iN; i0++) {
   ...
   if(iH[i0]->Integral() > 0.1 && i0 % 2  == 0) lL->AddEntry(iH[i0],fString[i0].c_str(),"p");
    if(iH[i0]->Integral() > 0.1 && i0 % 2  == 1) lL->AddEntry(iH[i0],fString[i0].c_str(),"l");
  }
  return lL;
}
