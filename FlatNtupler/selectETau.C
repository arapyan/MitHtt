...
void selectETau(const TString conf,         // input config file
  ...
  const Double_t kEleMass  = 0.000511;
  ...
  for(UInt_t isam=0; isam<samplev.size(); isam++) {
	...
	cout << "doing recoil corrections" << endl;
      ...
      eventTree->SetBranchAddress("Electron",     &eleArr);     TBranch *eleBr     = eventTree->GetBranch("Muon");
	...
	vector<const mithep::TElectron*> goodElev;
	  ...
	  if(!(ele->pt > 15 && ele->eta < 2.5)) continue;
	  if(passWP95(ele))
	    looseElev.push_back(ele);
	  ...
	  if(!(pass2012EleMVAID(ele, kMedium,0))) continue; 
	  goodElev.push_back(ele);
        }
	
	if(looseElev.size()-goodElev.size() > 0) continue;
	if(goodElev.size() > 2) continue;
	...
	for(Int_t i = 0; i < goodElev.size(); i++)
	  {
	    for(Int_t j = 0; j < goodHPSTaus.size(); j++)
	      {
	...
	if(issamesign) {
	  if(leadTau->q != leadEle->q) continue;
	}
	else {
	  if(leadTau->q == leadEle->q) continue;
	} 
	...
	lep1.SetPtEtaPhiM(leadEle->pt, leadEle->eta, leadEle->phi,kEleMass );
	lep2.SetPtEtaPhiM(leadTau->pt, leadTau->eta, leadTau->phi,leadTau->m);
	dilep = lep1+lep2;
	...
	l2.SetPtEtaPhi(leadEle->pt,0,leadEle->phi);
	...
	if(doRecoil) {
	  if(gen->id1a == EGenType::kTauElectron && gen->id2a == EGenType::kTauHadr)
	    {
	      pt1 = gen->pt_1_a;
	      pt2 = gen->pt_2_a;
	      eta1 = gen->eta_1_a;
	...
	lEta1		 = lep1.Eta();
	lM1		 = lep1.M();
	lIso1		 = eleIsoPU(leadEle);;
	lD01		 = leadEle->d0;
	lDZ1		 = leadEle->dz;
	lPassIso1	 = passEleIsoPU(leadEle);;
	...
	lMVAMt1		 = sqrt(2.0*(lep1.Pt()*mvamet*(1.0-cos(toolbox::deltaPhi(lep1.Phi(),mvametphi)))));
        ...
        lEta2		 = lep2.Eta();
        lM2		 = lep2.M();
        lIso2		 = leadTau->ringIso;
        lD02		 = leadTau->leadChargedHadronPFCand.d0;
        lDZ2		 = leadTau->leadChargedHadronPFCand.dz;
        lPassIso2	 = (leadTau->ringIso > 0.795);
